  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          CarouselSlider( items: Images.map((toElement)=>Container(
            width: double.infinity,
            margin: EdgeInsets.all(3),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(10),
              image: DecorationImage(image: AssetImage(toElement),fit: BoxFit.cover)
            ),
            ...
            onPageChanged: (index, reason) {
              setState(() {
                currentIndex = index; // Met à jour les points (dots)
              });
            },
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...Images.asMap().entries.map((item)=>Container(
                height: 7,
                width: 7,
                margin: EdgeInsets.all(4),
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  color: currentIndex == item.key ? Colors.blueAccent : Colors.grey,
                ),
              )).toList(),
